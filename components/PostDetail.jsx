...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case 'block-quote':
          return <div key={index} className="after:content=[''] after:h-full after:w-1 after:bg-pink-300 after:absolute after:left-0 after:top-0 relative : px-3 text-xl font-light mb-4">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</div>;
      case 'paragraph':
